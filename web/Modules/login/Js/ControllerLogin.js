...
async function loginUserSucursal(rol) {
    let user = document.getElementById('txtUser').value;
    let password = document.getElementById('txtPassword').value;
    ...
    if (user != "" && password != "") {
        ...
        if (response.response != "null" && (response.rol === rol || response.rol === "ADMS" || "EMPS")) {
            Swal.fire({
                title: 'Bienvenido a SICEFA Sucursal',
                //  text: 'Login Exitoso',
                html: '<b class="style-html">Login Exitoso</b>',
                icon: 'success',
                confirmButtonText: 'Ingresar',
